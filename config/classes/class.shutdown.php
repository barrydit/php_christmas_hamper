...
function dd($param = null, bool $die = true, bool $debug = true): void {
    ...
    $executionTime = defined('APP_END') ? round(APP_END - APP_START, 3) : round(microtime(true) - APP_START, 3);
    $output = "Execution time: <b>{$executionTime}</b> secs<br />\n";
    ...
    if ($die) {
        ...
        Shutdown::setEnabled(false)->setShutdownMessage(function() use ($param, $output) {
            return '<pre><code>' . htmlspecialchars(var_export($param, true)) . '</code></pre>' . $output;
        })->shutdown();
    } else {
        // If $die is false, output the dump and continue execution
        echo '<pre><code>' . htmlspecialchars(var_export($param, true)) . '</code></pre>' . $output;
    }
}
